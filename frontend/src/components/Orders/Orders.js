...
const Orders = () => {
   ...
   return (
      <div className="order-main-bg">
         ...
         <div
            className="table-responsive center order-bg"
            style={{ width: 1000 }}
         >
               ...
               <tbody>
                  {orders.map((order) => (
                     <tr key={order.orderId}>
                        {' '}
                        {/* Added a key prop for each row */}
                        <th scope="row">{`${order.orderId.substring(
                           0,
                           4,
                        )}**`}</th>
                        <th scope="row">{`${order.userId.substring(
                           0,
                           4,
                        )}**`}</th>
                        ...
                        <th scope="row">
                           {order.date.substring(0, order.date.length - 8)}
                        </th>
                     </tr>
                  ))}
               </tbody>
