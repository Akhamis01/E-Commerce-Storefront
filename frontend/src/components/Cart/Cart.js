...
export default function Cart() {
    ...
    const handleCheckout = () => {
        ...
        fetch("/clearcart").then(res => res.json()).then(data => {
            setCart([]);
        });
        fetch("/addpayment", {
            method:"POST",
            ...
            <div className="row no-gutters center">
                            ...
                            {cart.map((product, idx) => (
                                <div className="product shadow" key={product.id}>
                                    <div className="d-flex justify-content-between align-items-center mt-3 p-2 items rounded">
                                        <div className="d-flex flex-row"><img className="rounded" src={product.picture} width="100" alt=""/><i className="fa fa-trash-o ml-3 text-black-50"></i></div>
                                        <div className="ml-2"><span className="font-weight-bold d-block">{product.productName}</span></div>
                                        <span className="d-block ml-5 font-weight-bold">$ {(product.price).toFixed(2)} CAD</span>
                                        <div className="ml-2"><span className="font-weight-bold d-block">Quantity: {product.quantity}</span>
                                        </div>
                                        <button value={product.id} className={`btn btn-outline-info`} onClick={(e) => removeFromCart(e, product)}>Remove</button>
                                    </div>
                                </div>
                            ))}
                        </div>
                ...
                <div className="col-md-4">
                    <div className="payment-info">
                        <div className="d-flex justify-content-between align-items-center"><h4>Card details</h4></div>
                        <span className="type d-block mt-3 mb-1">Card type</span><label className="radio"> <input onClick={ (event) => setCardType(event.target.value)} type="radio" name="card" value="mastercard" defaultChecked /> <span><img width="30" src="https://img.icons8.com/color/48/000000/mastercard.png" alt=""/></span> </label>
                        <label className="radio"> <input onClick={ (event) => setCardType(event.target.value)} type="radio" name="card" value="visa" /> <span><img width="30" src="https://img.icons8.com/officel/48/000000/visa.png" alt=""/></span> </label>
                        ...
                        ((cardName && cardNum && cardDate && cardCVV) || (cardFound)) && (cart.length > 0) ? (
                            ...
                            <button onClick={handleCheckout} className="btn btn-primary btn-block d-flex justify-content-between mt-3" type="button" disabled>
                                <span>Checkout<i className="fa fa-long-arrow-right ml-1"></i></span>
                            </button>
                        )
                        }
