...
def send_password_reset_email(username, reset_token, email):
    ...
    msg = Message(
        "Password Reset Request", sender="cps714group19@gmail.com", recipients=[email]
    )
    msg.html = f"""
                <html>
...
def save_reset_token(userId, reset_token):
    ...
    db.collection("PasswordReset").document(userId).set(
        {"reset_token": reset_token, "expiry_time": reset_expiry}
    )
...
def verify_reset_token(reset_token):
    ...
    try:
        userId = serializer.loads(
            reset_token, salt=app.config["SECURITY_PASSWORD_SALT"], max_age=3600
        )
        return userId
...
def deleteProduct():
    ...
    for product in productsRef.stream():
        ...
        content = {
            ...
            "picture": productInfo["productImage"],
        }
        ...
        if not productID:
            try:
                for product in productsRef.stream():
                    ...
                    productID = hashlib.sha1(
                        tempProductName.encode("utf-8")
                    ).hexdigest()
                    break
            except:
                return jsonify(alert="error")
...
def addPayment():
    if request.method == "POST":
        ...
        if session["isAdmin"] != True:
            ...
            existing_payment = (
                user_payments_ref.where("userId", "==", userId).limit(1).stream()
            )
...
def removeFromCart():
    if request.method == "POST":
        ...
        if session["isAdmin"] != True:
            query = cartsRef.where("productId", "==", productID).where(
                "userId", "==", userID
            )
...
def removeFromInventory():
    ...
    if session["isAdmin"] != True:
        ...
        for productId in productIdsArray:
            ordersId = hashlib.sha1(
                f"{userId}{random.randint(0, 1000)}".encode("utf-8")
            ).hexdigest()
            productsRef.document(productId).update(
                {"stockQuantity": firestore.Increment(-1)}
            )
...
def login():
    if request.method == "POST":
        ...
        if username == userInfo["username"] and hashPassword == userInfo["password"]:
            ...
            response_data = {"alert": "success", "isVerified": is_verified}
...
def register():
    if request.method == "POST":
        ...
        if userInfo is None:
            ...
            msg = Message(
                "Email Verification",
                sender="cps714group19@gmail.com",
                recipients=[email],
            )
            msg.html = f"""
                            <html>
...
def verify():
    if request.method == "POST":
        ...
        user_doc = db.collection("Users").document(
            hashlib.sha1(username.encode("utf-8")).hexdigest()
        )
